...
namespace Media
{
	...
	void Decoder::FrameUnref(const DataType::Frame& frame)
	{
		auto buff = frame.get();
		av_frame_unref(buff);
	}

	void Decoder::PacketUnref(const DataType::Packet& packet)
	{
	...
	float Decoder::GetCurrentFileDuration()
	{
	...
	Error Decoder::SetStreamDecoder(DataType::Stream& stream, DataType::StreamDecoder& decoder)
	{
		...
		decoder = std::unique_ptr<AVCodecContext>(avcodec_alloc_context3(streamCodec.get()));
